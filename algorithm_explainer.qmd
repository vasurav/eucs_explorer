...
1.  USAU starts everyone with a rating of 1000, so scores are typically between 0 and 2000. In the EUCS implementation everyone starts with a score of 0, so typical scores are between -1000 and 1000. If you want an equivalent USAU rating, add 1000 to the ranting here.
...
The rating is calculated by taking the rating of the opponent and then adding or subtracting a differential based on the score difference of the game. You add the differential if you win the game and subtract the differential if you lose the game. The rating for the score difference is calculated below:

$$\small 125+475\frac{\sin{\left(\min{\left(1, \frac{1-r}{0.5}\right)}\cdot 0.4 \pi\right)} }{\sin{\left(0.4 \pi\right)}}$$

$$r=\frac{losing score}{winning score-1} $$

This the following important properties:
...
See plot and tables showing the rating difference below.

So as an example if you beat a team with a rating of 302 15-14, you would earn a game rating for that game of 302 + 125 = 427. If you would lose to the same team 14-15 you would earn a game rating of 302 - 125 = 177.
...
1. All games that go to at least 13 are counted fully
...
1. The winning team has at least 5 other results where results are not being ignored. If removing multiple games would bring the winning team below 5 games (for example, 5 blowout elgible wins in 8 total games), then the first games removed are the ones that hurt the winning team the most.
